...
for(int j=0; j<msh->NY; j++){
    ...
    DvqDy=(solver->u[n + msh->Nnodes]*solver->q[n]-solver->u[msh->N2N[4*n+3]+msh->Nnodes]*solver->q[msh->N2N[4*n+3]])/(msh->y[n]-msh->y[msh->N2N[4*n+3]]);
    }else{ 
    DvqDy=(-solver->u[n + msh->Nnodes]*solver->q[n]+solver->u[msh->N2N[4*n+1]+msh->Nnodes]*solver->q[msh->N2N[4*n+1]])/(-msh->y[n]+msh->y[msh->N2N[4*n+1]]);
    }
